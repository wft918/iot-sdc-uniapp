	...
	<view class="container">
		<view class="infoOut" style="margin-top: 16rpx;">
			...
			<view class="inputInfo">
				<view class="inputInfoL"><text style="color:#aa282e;">*</text>问题分类</view>
				<view class="inputInfoR" @tap="toQueSClass()">
					<view class='inputInfoRPack'>
						<input type="text" :value="questionClass.name" placeholder="请选择" placeholder-style="color:#aaa;"
							disabled="true" />
					</view>
			...
			<view class="inputInfo">
				...
				<view class="inputInfoR">
					<input type="text" placeholder="输入问题标题" @confirm="closeInput" v-model="questionTitle" class="inart"
			...
			<view style="padding-bottom: 20rpx;">
				<textarea class="quesContent1" placeholder-style="color:#aaa;" placeholder="输入问题描述"
		...
		<view class="infoOut" style="margin-top: 16rpx;">
			...
			<view class="inputInfo">
				<view class="inputInfoL"><text style="color:#aa282e;">*</text>检查位置</view>
				<view class="inputInfoR">
					<input type="text" @confirm="closeInput" placeholder="输入位置" v-model="positionName" class="inart"
						placeholder-style="color:#aaa;" />
				</view>
			</view>
			<view class="inputInfo">
				...
				<view class="inputInfoR">
					<picker mode="date" :style="date?'':'color:#aaa'" class='inputInfoRPack' :value="date"
			...
			<view class="inputInfo" @tap="secPerson()">
				...
				<view class="inputInfoR">
					<view class='inputInfoRPack'>
						<input type="text" :value="secPersonInfo.username" placeholder="选择整改人"
...
<script>
	...
	export default {
		...
		onShow() {
			...
			uni.$on("haveClass", res => {
				console.log(res)
				this.questionClass = res;
				// 清除监听
				uni.$off('haveClass');
			})
		},
		methods: {
			...
			toQueSClass() {
				...
				// let id=this.list[this.index1].projectId||'';
				// let name=this.list[this.index1].projectName||'';
				// console.log(id,name);
				uni.navigateTo({
					url: 'questionClassfication/questionClassfication'
				})
			},
			getTime() {
				var date = new Date(),
			...
			sub(st) {
				...
				strs = this.questionClass.parentIds.split(","); //字符分割
				console.log(strs);
				let checkId = strs[1] + ',' + strs[2] + ',' + strs[3] + ',' + this.questionClass.id;
